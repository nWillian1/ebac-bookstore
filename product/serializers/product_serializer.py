...
from product.models.product import Product, Category # importando o modelo Product do aplicativo product
...
class ProductSerializer(serializers.ModelSerializer): # criando um serializer para o modelo Product, que é uma subclasse de ModelSerializer
    ...
    def create(self, validated_data): # definindo um método para criar um produto a partir dos dados validados.
        category_data = validated_data.pop("categories_id") # extraindo os dados das categorias do dicionário de dados validados.
        validated_data.pop("category", None) # removendo o campo de categoria do dicionário de dados validados, caso ele exista, para evitar conflitos com o campo de IDs de categorias.
        product = Product.objects.create(**validated_data) # criando um novo produto com os dados validados.
        
        for category in category_data: # iterando sobre a lista de IDs de categorias.
            product.category.add(category) # adicionando cada categoria ao produto usando o método add() do campo de categoria do modelo Product.
