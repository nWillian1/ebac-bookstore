...
class OrderSerializer(serializers.ModelSerializer): # criando um serializer para o modelo Order, que é uma subclasse de ModelSerializer
    ...
    products_id = serializers.PrimaryKeyRelatedField(queryset=Product.objects.all(), write_only=True, many=True) # definindo um campo de IDs de produtos, que é uma lista de IDs de produtos. Este campo é write-only, ou seja, só pode ser usado para criar ou atualizar um pedido, e não será incluído na representação do pedido.
    ...
    class Meta: # definindo a classe Meta para o serializer, que é usada para configurar o comportamento do serializer.
        ...
        fields = ["product", "total", "user", "products_id"] # definindo os campos que o serializer irá usar, que são o campo de produtos e o campo de total.
        extra_kwargs = {"product": {"required": False}} # definindo que o campo de produtos não é obrigatório, ou seja, pode ser omitido na criação de um pedido.

    def create(self, validated_data): # definindo um método para criar um pedido a partir dos dados validados.
        product_data = validated_data.pop("products_id") # extraindo os dados dos produtos do dicionário de dados validados.
        user_data = validated_data.pop("user") # extraindo os dados do usuário do dicionário de dados validados.
        order = Order.objects.create(user=user_data) # criando um novo pedido com os dados do usuário.

        for product in product_data: # iterando sobre a lista de IDs de produtos.
            order.product.add(product) # adicionando cada produto ao pedido usando o método add() do campo de produtos do modelo Order.
