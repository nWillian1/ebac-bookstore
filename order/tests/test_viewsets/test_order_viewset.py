...
class TestOrderViewSet(APITestCase):
    ...
    def setUp(self):
        self.user = UserFactory() # criando um usuário usando a fábrica de usuários para associar ao pedido.
        ...
        self.order = OrderFactory(product=[self.product], user=self.user) # Cria um pedido usando a fábrica de pedidos para testar a criação do pedido.
    ...
    def test_order(self):
        ...
        order_data = json.loads(response.content) # Carrega o conteúdo da resposta como JSON.
        self.assertEqual(order_data["results"][0]["product"][0]["title"], self.product.title) # Verifica se o título do produto no pedido é igual ao título do produto criado na configuração do teste.
        self.assertEqual(order_data["results"][0]["product"][0]["price"], self.product.price) # Verifica se o preço do produto no pedido é igual ao preço do produto criado na configuração do teste.
        self.assertEqual(order_data["results"][0]["product"][0]["active"], self.product.active) # Verifica se o título do produto no pedido é igual ao título do produto criado na configuração do teste.
        self.assertEqual(order_data["results"][0]["product"][0]["category"][0]["title"], self.category.title) # Verifica se o título da categoria do produto no pedido é igual ao título da categoria criada na configuração do teste.
